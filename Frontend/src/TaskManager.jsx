...
function TaskManager() {
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      console.error(err);
      alert(`failed to create the task2, error ${err}`);
    }
  };
  ...
  const fetchAllTasks = async () => {
    try {
      ...
      alert(err);
      notify(`failed to create the task3, error ${err}`);
    }
  };
  useEffect(() => {
    fetchAllTasks();
  ...
  const handleCheckAndUncheck = async (item) => {
    ...
    try {
      ...
      alert(err);
      notify("failed to do task", "error");
    }
  };
  ...
  const handleUpdateItem = async (item) => {
    ...
    try {
      ...
      alert(err);
      notify("Failed to create task4", "error");
    }
  };
